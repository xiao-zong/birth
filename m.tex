...
Since a subset of $S$ can equivalently be represented as a non-negative integer-valued measure, we consider the space
$\mathcal{N}$ of locally finite non-negative integer-valued measures on $(S,\mathscr{B}(S))$.
...
Define $\mathscr{H}$ as the smallest $\sigma$-field on $\mathcal{N}$ such that the mappings $\mu\mapsto\mu(B)$ are measurable for all relatively compact sets $B$ in $\mathscr{B}(S)$~\cite[\S1.1]{kallenberg83}.
...
$$\N_{>n}:=\{n+1,n+2,\dots\}.$$
\subsection*{Markov processes and martingale problem}
 
Let $E$ be a metric space. Denote by $\mathcal{M}(E)$ the collection of all real-valued Borel measurable functions on $E$, and  
$\mathcal{B}(E) \subset \mathcal{M}(E)$ the Banach space of bounded functions with 
$\|f\| := \sup_{x \in E} |f(x)|,$
and $\overline{\mathcal{C}}(E) \subset \mathcal{B}(E)$ is the subspace of bounded continuous functions.
\begin{prop}\cite[Theorem~4.9.9]{EK09}\label{statdis1}
Let $E$ be locally compact and separable. 
Let $\{T(t)\}$ be a semigroup on $\mathcal{B}(E)$ given by a transition function $P(t,x,\Gamma)$ such that 
$T(t): \overline{\mathcal{C}}(E) \to \overline{\mathcal{C}}(E)$ for all $t\ge0$,
and let $X$ be a measurable Markov process corresponding to $\{T(t)\}$.
Let $\varphi, \psi \in \mathcal{M}(E)$, $\varphi \ge 0$, $\psi \le C$ for some constant $C$, and 
$\lim_{x \to \Delta} \psi(x) < 0,$
and suppose that 
\begin{equation}
	\varphi(X(t))-\int_0^t\psi(X(s))ds
\end{equation}
is a supermartingale. 
Then there exists a stationary distribution for $\{T(t)\}$.
\end{prop}
...
$(S^\ell, \mathcal{S}^{\otimes \ell})$ such that:
\begin{enumerate}
  ...
  for each $F \in \mathcal{S}^{\otimes \ell}$ the probability that their 
  locations lie in $F$ during the interval $[t, t+s]$ is given by
  \[
    b_\ell(\xi, F)\, s + o(s).
  \]
\end{enumerate}
	\item If there are $n$ individuals alive at time $t$, with locations $\xi=\{x_1,\dots,x_n\}$, then there exits a $\mathcal{S}$-measurable function $D(\xi,\cdot): S\to[0,\infty)$ such that the probability of the individual at the point $x_i\in\xi$ dying between times $t$ and $t+s$ is $D(\xi,x_i)s+o(s)$;
...
Denote $\nu\in\mathscr{P}(\mathcal{N})$ a probability distribution of a Gibbs point process on $S$.
...
$b_\ell : \mathcal{N} \times \mathcal{S}^{\otimes \ell} \to [0,\infty)$ be a kernel such that, for every 
$\xi \in \mathcal{N}$, the measure $b_\ell(\xi, \cdot)$ is symmetric on 
$(S^\ell, \mathcal{S}^{\otimes \ell})$. Define
\begin{align}
    b(\xi) &:= \sum_{\ell=1}^{\infty} \ell\, \lambda_\ell(\xi)\, b_\ell(\xi, S^\ell),
\end{align}
for any $\xi \in \mathcal{N}$. Furthermore, define
\begin{align}
    d(\xi) &:= \sum_{x \in \xi} D(\xi, x),
\end{align}
for all $\xi \neq \emptyset$, and set $d(\emptyset) := 0$.  
Finally, denote
\[
    \lambda(\xi) := b(\xi) + d(\xi).
\]
...
\begin{equation}
	Y_{n+1}:=\begin{cases}
		\xi\cup\{x_1,\dots,x_\ell\},\ \ \ &\mbox{w.p.}~\frac{1}{\lambda(\xi)}\ell\lambda_{\ell}(\xi)b_{\ell}(\xi,\mathrm{d}x_1,\dots,\mathrm{d}x_{\ell});\\
		\xi\backslash\{x\},\ \ \ &\mbox{w.p.}~\frac{D(\xi,x)}{\lambda(\xi)};
	\end{cases}
\end{equation}
\item That is $\{Y_n\}_{n\ge0}$ has initial distribution $\nu$ and the transition function
\begin{align}
	K(\xi,M):&=\IP(Y_{n+1}\in M|Y_n=\xi)\nonumber\\
		  &=\frac{1}{\lambda(\xi)}\sum_{\ell=1}^{\infty}\ell\lambda_{\ell}(\xi)\int_{S^{\ell}}\bone(\xi\cup\{x_1,\dots,x_\ell\}\in M)b_{\ell}(\xi,\mathrm{d}x_1,\dots,\mathrm{d}x_{\ell})\nonumber\\
		  &\ \ \ \ +\frac{1}{\lambda(\xi)}\sum_{x\in\xi}D(\xi,x)\bone(\xi\backslash\{x\}\in M).\label{trans1}
\end{align}
\end{enumerate}
...
Denote $\lambda:=\sup_{\xi\in\mathcal{N}}\lambda(\xi)$. To avoid trivialities, assume that $\lambda>0$. Define the transition function $K'(\xi,M)$ on $\mathcal{N}\times \mathscr{H}$ by 
\begin{equation}
	K'(\xi,M):=\left(1-\frac{\lambda(\xi)}{\lambda}\right)\delta_{\xi}(M)+\frac{\lambda(\xi)}{\lambda}K(\xi,M),
\end{equation}
with $K$ defined in \eqref{trans1}.


Given $X_t=\xi$ with $\xi\in\mathcal{N}$ and $t\ge0$, then the waiting time to the next jump has an exponential distribution with expectation $1/\lambda(\xi)$ and is independent of the past history. The subsequent jump leads either to a death of an individual located at point $x\in\xi$ with probability $\frac{D(\xi,x)}{\lambda(\xi)}$, or a birth of $\ell$ individuals, distributed according to $b_{\ell}(\xi,\cdot)$.  
...
\red{
\begin{equation}
	\sum_{\ell=1}^\infty \ell \lambda_\ell(\xi)\int_{S^\ell}\left(f(\xi\cup\{x_1,\dots,x_\ell\})-f(\xi)\right)b_\ell(\xi,\mathrm{d}x_1\cdots\mathrm{d}x_\ell)+\int_{S}D(\xi,x)\left(f(\xi\backslash\{x\})-f(\xi)\right)\xi(\mathrm{d}x).
\end{equation}}
...
$Q_t$ is defined to be the solution of Kolmogorov's backwards equations
...
Set for any $n\ge1$, 
\begin{align}
	q_{n,n-1}&:=\inf_{\xi:\|\xi\|=n}d(\xi)=\inf_{\xi:\|\xi\|=n}\sum_{x\in\xi}D(\xi,x);\label{qmatrix1}\\
	q_{i,j}&:=0,~~~\forall~j<i-1;\\
	q_{n,n+\ell}&:=\sup_{\xi:\|\xi\|=n}\ell b_{\ell}(\xi,S^{\ell}),~~~\forall~\ell\ge1;\\
...
\begin{thm}
	Suppose the single death process $\{Z_t\}_{t\ge0}$ with $Q$-matrix given in \eqref{qmatrix1}-\eqref{qmatrix4} satisifying 
$\sum_{n=1}^\infty R_n=\infty$, where $R_0=R_{-1}=1$ and for all $n\ge1$, 
\begin{equation}
	R_n:=\frac1{q_{n+1,n}}\left(1+\sum_{m=0}^n\sum_{k=n+1}^{\infty}q_{m,k}R_{m-1}\right),
\end{equation}
then the backward equations \eqref{backweq-1} and \eqref{backweq-2} corresponding to $\lambda$ and $K$ have a unique solution.
\end{thm}
The theorem is a direct result of Theorem~\ref{thm-uni1} and \cite[Theorem~4]{CPZC05}.
\begin{thm}
	If there exists a stationary distribution for the simple process $\{Z_t\}_{t\ge0}$, then there also exists a stationary distribution for $\{X_t\}_{t\ge0}$.   
\end{thm}
\begin{proof}
	According to \cite[Theorem~7]{CPZC05}, a downwardly skip-free process with a conservative $q$-matrix $Q=(q_{i,j})_{i,j\ge0}$ has a  unique stationary distribution if and only if $\sum_{n=1}^\infty R_n=\infty$, where $R_0=R_{-1}=1$ and for all $n\ge1$, 
\begin{equation}
	R_n:=\frac1{q_{n+1,n}}\left(1+\sum_{m=0}^n\sum_{k=n+1}^{\infty}q_{m,k}R_{m-1}\right).
\end{equation}
According to Proposition~\ref{statdis1}, 
